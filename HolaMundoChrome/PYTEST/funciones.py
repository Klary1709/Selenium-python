...
#Clickear elementos
    ...
    def activar_iframe(self,tipo,frame, tiempo=0): 
        """
        Este metodo permite dar clic a un elemento especificando su tipo de indicador y indicador unico, y arrastrarlo dentro de un iframe 
            tipo: xpath,id,css
            frame: el identificador único del frame 
    ...
    def hover_mixto(self,tipo,elemento,accion, tiempo=0): 
        """
        Este metodo permite realizar la accion de hover a un elemento especificando su tipo de indicador y indicador unico
            tipo: xpath,id,css
            elemento: el identificador único del elemento 
            tiempo: tiempo de retraso 
        """
        ...
        if(tipo.lower()=="xpath"):
            ...
            try:
                valor=self.SEXP(elemento)
                accion.move_to_element(valor).perform()
                print("hover en el campo->> "+elemento)
                t=time.sleep(tiempo)
                return t
            except TimeoutException as ex:
                print("\nNo se ha encontrado el elemento que se quiere clickear con el XPATH ingresado, posiblemente el xpath este erroneo " 
...
#Funciones para cargar archivos 
    def subirarch_xpath(self, ruta ="C://agregarLaDoblefleca al incio de la url", xpath="" ,  tiempo=0 ):
        """
    ...
    def subirarch_id(self, ruta ="C://agregarLaDoblefleca al incio de la url" ,id="id",  tiempo=0 ):
        """
        NOTA:  SE DEBE AGREGAR LA DOBLE // EN EN LA URL     
    ...
    def subirarch_csselector(self, ruta ="C://agregarLaDoblefleca al incio de la url" ,css="cssSelector" ,  tiempo=0 ):
        """
        NOTA:  SE DEBE AGREGAR LA DOBLE // EN EN LA URL     
...
#COMANDOS BASICOS
    ...
    def TABULAR(self, acciones,texto="", tiempo=0):
        ...
        Args:
            acciones (ActionChains): esta variable debe ser tipo ActionChain y debe ser declarada en el test que se va usar, no se puede declarar de forma global o en metodos inicializadores 
            tiempo (int es optional): Defaults to 0.
            texto (str): se puede anexar un texto despues del tabular
            
        ejemplo de aplicacion: 
            se debe declar esta variable y luego solo se pasa 
            acciones=ActionChains(d)
            // f.TABULAR(acciones)
        """
        #acciones=ActionChains(self.driver)
        if(texto==""):
            ...
            acciones.send_keys(Keys.TAB + texto)
            tiempo=time.sleep(tiempo)
            print("Se presiono la tecla TAB y se ingreso el siguiente texto :"+texto)
